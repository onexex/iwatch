    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 p-4 md:flex-row">
            <div class="relative min-h-[500px] flex-1 overflow-hidden rounded-xl border bg-muted/50">
                ...
                <div v-if="!isHeatmapEnabled" class="absolute bottom-6 left-6 z-[1000] rounded-lg border bg-background/90 p-3 shadow-sm backdrop-blur">
                    <h4 class="mb-2 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Classification</h4>
                    <div class="space-y-1.5">
                        <div class="flex items-center gap-2 text-[11px] font-medium"><span class="h-3 w-3 rounded-full bg-[#dc2626]"></span> TG1</div>
                        <div class="flex items-center gap-2 text-[11px] font-medium"><span class="h-3 w-3 rounded-full bg-[#2563eb]"></span> TG2</div>
                        <div class="flex items-center gap-2 text-[11px] font-medium"><span class="h-3 w-3 rounded-full bg-black"></span> CISS</div>
                        <div class="flex items-center gap-2 text-[11px] font-medium"><span class="h-3 w-3 rounded-full bg-[#eab308]"></span> Wanted</div>
                        <div class="flex items-center gap-2 text-[11px] font-medium"><span class="h-3 w-3 rounded-full bg-[#16a34a]"></span> Other</div>
                    </div>
                </div>
        ...
        <div v-show="!isSidebarCollapsed" class="p-4 pt-0 space-y-4 animate-in fade-in slide-in-from-top-2 duration-200">
            ...
            <div class="mt-2 mb-4 flex items-center justify-between rounded-lg border bg-muted/50 p-2">
                ...
                <button 
                    ...
                    :class="[isHeatmapEnabled ? 'bg-orange-600' : 'bg-slate-300']"
            ...
            <div class="mt-2 space-y-2">
                ...
                >
                    📥 Export ({{ filteredIncidents.length }})
                </button>
            </div>
        </div>
    </div>
...
import 'leaflet.heat'; // Add this import here
...
export default defineComponent({
    ...
    setup() {
        ...
        const searchedIncidents = computed(() => {
            ...
            if (!query) return filteredIncidents.value;
    ...
    if (isHeatmapEnabled.value) {
        // Ensure values are numbers for the heatmap
        ...
        // Access via (L as any) to avoid TypeScript errors
        ...
        onMounted(async () => {
         ...
         L.control.layers(baseMaps, {} , {
            position: 'bottomright' // You can also use 'bottomleft'
        }).addTo(map);
